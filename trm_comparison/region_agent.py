"""Region-based Q-learning agent for the LB-TRM Frozen Lake task.

State key : (grid_pos, rm_state, x_capped, y_capped)
            x_capped = min(x, c_min)  — per-phase clock capped at lower-bound threshold
            y_capped = min(y, D)      — global clock capped at deadline

Action    : (delay, grid_action)   delay ∈ {0,..,d_max}, grid_action ∈ {0..3}

Region semantics
----------------
For guard  x >= c_min AND y <= D:
  - x is tracked exactly up to c_min; all x >= c_min share one class.
    Gives c_min+1 distinct x states: 0, 1, ..., c_min.
  - y is tracked exactly up to D; all y > D share one class.
    Gives D+1 distinct y states: 0, 1, ..., D.
Total clock-state pairs: (c_min+1) × (D+1), which grows linearly with c_min.
Zone, by contrast, always has exactly 4 clock-state pairs regardless of c_min.

Q-learning update
-----------------
  r = reward from TRM step
...
class TRMRegionEnv:
    """TRM Frozen Lake simulation with integer-clock region states."""
    ...
    def state_key(self) -> tuple:
        x_cap = min(self._x, self.trm.c_min)
        y_cap = min(self._y, self.trm.D)
        return (self._grid.pos(), self._trm_s.u, x_cap, y_cap)


class TRMRegionAgent:
    """Tabular Q-learning over (pos, rm_state, x_capped, y_capped) × (delay, grid_action).

    Parameters
    ----------
    trm           TRM instance
    d_max         Maximum delay the agent can choose
    alpha         Learning rate
    ...
    def __init__(
        ...
        self._actions: List[Tuple[int, int]] = [
            (d, a) for d in range(d_max + 1) for a in ACTIONS
        ]
