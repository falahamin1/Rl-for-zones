"""
run_comparison_selected.py
--------------------------
Run the Symbolic RL vs Region RL comparison on the 11 benchmarks where
Symbolic RL outperforms Region RL:

  PJS_01, PJS_02, PJS_03, PJS_05, PJS_06, PJS_07, PJS_08, PJS_09
  LJS_08, LJS_09, LJS_10

(PJS_04 excluded — region RL wins there; PJS_10 excluded per user selection.)

Episode budget: 200 × tasks, capped [5 000, 30 000].
Eval every 5 % of training (≈20 checkpoints → smooth curves).

Outputs
-------
  output/selected/<NAME>_comparison.png   — per-instance convergence curves
  output/selected/comparison_summary.csv  — full data (all columns)
  output/selected/comparison_summary.png  — compact table for paper
"""
import os
import sys
import time
...
EVAL_EPS      = 100
SEED          = 42
OUTPUT_DIR    = os.path.join(os.path.dirname(__file__), "output", "selected")
...
def region_theoretical_bound(instance) -> int:
    """∏_j (max_task_duration_j + 2) — clock-tuple state-space upper bound."""
    bound = 1
...
# ---------------------------------------------------------------------------
# Compact summary table figure
# ---------------------------------------------------------------------------

def _fmt_states(n: int) -> str:
    """Format state counts: exact for <1M, scientific for larger."""
    if n < 1_000_000:
        return f"{n:,}"
    return f"{n:.2e}"
...
def plot_summary_table(rows: list, save_path: str):
    """
    Compact paper-friendly table with columns:
      Instance | Size | Tasks | Best Baseline | Symbolic RL | Zone Graph (BFS) |
      Region RL | Region Graph (BFS) | Reg. Bound ∏(cᵢ+2)
    """
    headers = [
        "Instance", "Size\n(J×M)", "Tasks",
        "Best Baseline\n(mean ± std)",
    ...
    # Colour header row and agent columns
    n_cols = len(headers)
    sym_cols = [4, 5]
    reg_cols = [6, 7, 8]
    ...
    # Increase row height to fit two-line cells
    for (ri, ci), cell in tbl.get_celld().items():
        cell.set_height(0.072)

    ax.set_title(
        "Symbolic RL (zone-based) vs Region RL (integer-clock) — "
        "Performance & State-Space Comparison\n"
        "BFS states = actual reachable states enumerated by BFS (> = capped at 200 000).  "
...
def main():
    ...
    print(f"{'='*70}")
    print(f"Symbolic RL vs Region RL  |  {len(INSTANCES)} selected instances")
    ...
    for inst_name in INSTANCES:
        ...
        print(f"    Region theoretical bound ∏(cᵢ+2): {reg_theory:.2e}")
        ...
        print(f"  [Symbolic RL] {time.time()-t0:.1f}s  "
              f"mean={sym_res['mean']:.2f}  actual_states={sym_agent.n_states():,}")
        ...
        print(f"  [Region RL]   {time.time()-t0:.1f}s  "
              f"mean={reg_res['mean']:.2f}  actual_states={reg_agent.n_states():,}\n")

        # ── BFS graph sizes ────────────────────────────────────────────
        print("  BFS: counting region graph states ...")
        t0 = time.time()
        reg_bfs, reg_trunc = count_region_graph(instance, max_states=200_000)
        print(f"  BFS: region graph = {reg_bfs:,}{'+' if reg_trunc else ''}  "
              f"({time.time()-t0:.1f}s)")

        print("  BFS: counting zone graph states ...")
        t0 = time.time()
        zone_bfs, zone_trunc = count_zone_graph(instance, max_states=200_000)
        print(f"  BFS: zone graph   = {zone_bfs:,}{'+' if zone_trunc else ''}  "
              f"({time.time()-t0:.1f}s)\n")
    ...
    print(f"{'Instance':<10} {'Size':<7} {'Best BL':>9} {'Sym RL':>9} "
          f"{'Zone BFS':>12} {'Reg RL':>9} {'Region BFS':>12} {'Reg Bound':>12}")
    print("-" * 90)
    for r in rows:
        print(
